...
export class RoomSettingComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    Promise.resolve().then(() => this.modalService.title = this.panelService.title = '建立房間');
  ...
  calcPeerId(roomName: string, password: string) {
    ...
    const peer = PeerContext.create(userId, PeerContext.generateId('***'), roomName, password);

  ...
  show(gameCharacter: GameCharacter, standElement: DataElement, color: string | null = null, isSecret=false) {
    ...
    for (const pair of StandImageService.currentStandImageShowing) {
      ...
      if (instance.isVisible && gameCharacter.identifier != identifier) {
        ...
        instance.color = color ? color : gameCharacter.chatPalette?.color || '#000000';
    ...
    if (isNewbee && gameCharacter.location.name != 'graveyard') {
      ...
      standImageComponentRef.instance.color = color ? color : gameCharacter.chatPalette?.color || '#000000';

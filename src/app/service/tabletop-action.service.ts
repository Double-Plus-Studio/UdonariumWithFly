...
export class TabletopActionService {
  ...
  private cardName(code: string) {
    ...
    if (suit == 'x') {
      ...
      ret += ` ${number == 1 ? 'A' : number >= 11 ? jqk[number - 11] : number }`
    }
    return ret;
  }
  ...
  createRangeArea(position: PointerCoordinate, typeName: string): RangeArea {
    ...
    switch (typeName) {
      case 'LINE':
        range = RangeArea.create('射程・範圍 (直線)', 1, 6, 100);
        break;
      case 'CIRCLE':
        range = RangeArea.create('射程・範圍 (円)', 3, 3, 100);
        break;
      case 'SQUARE':
        range = RangeArea.create('射程・範圍 (正方形)', 3, 3, 100);
        break;
      case 'DIAMOND':
        range = RangeArea.create('射程・範圍 (菱形)', 3, 3, 100);
        break;
      case 'CORN':
      default:
        range = RangeArea.create('射程・範圍 (錐形)', 6, 6, 100);
        break;
    }

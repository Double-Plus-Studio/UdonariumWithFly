...
export class SaveDataService {
  ...
  private _saveRoomAsync(fileName: string = 'fly_房間資料', updateCallback?: UpdateCallback): Promise<void> {
    ...
    const diceRollTableXml = this.convertToXml(DiceRollTableList.instance);
    const cutInXml = this.convertToXml(CutInList.instance);
  ...
  private searchImageFiles(xml: string): ImageFile[] {
    ...
    for (let i = 0; i < imageElements.length; i++) {
      ...
      const shadowIdentifier = imageElements[i].getAttribute('currentValue');
    ...
    for (let i = 0; i < imageElements.length; i++) {
      ...
      const backgroundImageIdentifier2 = imageElements[i].getAttribute('backgroundImageIdentifier2');

...
export class CutInService {
  ...
  play(cutIn: CutIn, isSecret=false, isTest=false, sender: string | null = null) {
    if (!cutIn) return;
    for (const cutInComponentRef of CutInService.cutInComponentRefQueue) {
      if (cutInComponentRef && cutInComponentRef.instance) {
        ...
        if (cutInComponent.isEnd) {
          ...
          if (tmp && (cutIn.identifier === tmp.identifier || (!!cutIn.videoId && !!tmp.videoId) || (cutIn.tag != null && cutIn.tag.trim() != '' && tmp.tag != null && tmp.tag.trim() != '' && StringUtil.toHalfWidth(cutIn.tag).toUpperCase().trim() == StringUtil.toHalfWidth(tmp.tag).toUpperCase().trim()))) {
            cutInComponent.stop();
          }
        }
      }
